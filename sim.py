...
C_index_AR, C_index_AL, C_index_gAL, C_index_GWAL, C_index_GPAL, C_index_LWGAL = \
    np.zeros((effective_repeat, M)), np.zeros((effective_repeat, M)), \
    np.zeros((effective_repeat, M)), np.zeros((effective_repeat, M)), \
    np.zeros((effective_repeat, M)), np.zeros((effective_repeat, M)) # Used to store the error of each method in repeated experiments
coef_MAB_loss = np.zeros([6, effective_repeat])
coef_MSE_loss = np.zeros([6, effective_repeat])
TP = np.zeros([5, effective_repeat])
Model_Size = np.zeros([5, effective_repeat])
Cindex = np.zeros([6, effective_repeat])
...
for random_state in range(repeat_number):
    try:
        ...
        relationship_matrix = Relationship_matrix(data, alpha=alpha, theta=theta, threshold=threshold)
        ...
        print("GPAL_model (The best lamda):", lamda[np.argmax(cindex_par[1, :][0::len(bw)])])
        print("LWGAL_model (The best lamda):", lamda[np.argmax(cindex_par[1, :]) // len(bw)],
              "(The best bw):", bw[np.argmax(cindex_par[1, :]) % len(bw)])

        print('###############Start conducting the test set########################')
        AR_coef_loss, AL_coef_loss, gAL_coef_loss, GWAL_coef_loss, GPAL_coef_loss, LWGAL_coef_loss \
            = [], [], [], [], [], []
        AR_coef, AL_coef, gAL_coef, GWAL_coef = [], [], [], []
        AR_intercept, AL_intercept, gAL_intercept, GWAL_intercept = [], [], [], []
        AL_select, gAL_select, GWAL_select, GPAL_select, LWGAL_select = [], [], [], [], []
        ...
        gwr_bw = bw[0]
        Aerfa = Kernel(distance_matrix, gwr_bw, dataset, 'bi-square')
        GPAL_model = LWGAL(lamd=lamda[np.argmax(cindex_par[1, :][0::len(bw)])], tol=0.001, max_iter=1000, threshold=threshold, verbose=False)
        GPAL_model.fit(X_train_o, y_train_o, Aerfa, datasets, p, M)

        # if check_hyperparameter(lamda[np.argmax(cindex_par[1, :][0::len(bw)])], gwr_bw, lamda, bw):
        #     continue
        ...
        for loc in range(len(dataset)):
            ...
            if len(XX_test) == 0:
                continue
            ####################################GPAL####################################
            y_GPAL = GPAL_model.predict(add_intercept(XX_test), loc, p).flatten()
            C_index_GPAL[eff, loc] = concordance_index(yy_test[:, 1], y_GPAL, event_observed=yy_test[:, 0])
        ...
        print('C_index_GPAL', np.average(C_index_GPAL[eff, :]))
        ...
        GPAL_coef_loss.append(Criteria().estimation_process(np.array(GPAL_model.coef_.reshape((M, p))),np.array(GPAL_model.intercept_), beta))
        GPAL_select.append(Criteria().selection_process(np.array(GPAL_model.coef_.reshape((M, p))), beta))
        ...
        print("Simulation", eff + 1, "ARcl, ALcl, gALcl, GWALcl, GPALcl, LWGALcl", end='')
        print(AR_coef_loss, AL_coef_loss, gAL_coef_loss, GWAL_coef_loss, GPAL_coef_loss, LWGAL_coef_loss)

        print("Selection")
        print(AL_select, gAL_select, GWAL_select, GPAL_select, LWGAL_select)

        coef_MAB_loss[0, eff] = AR_coef_loss[0][0]
        coef_MAB_loss[1, eff] = AL_coef_loss[0][0]
        coef_MAB_loss[2, eff] = gAL_coef_loss[0][0]
        coef_MAB_loss[3, eff] = GWAL_coef_loss[0][0]
        coef_MAB_loss[4, eff] = GPAL_coef_loss[0][0]
        coef_MAB_loss[5, eff] = LWGAL_coef_loss[0][0]

        coef_MSE_loss[0, eff] = AR_coef_loss[0][1]
        coef_MSE_loss[1, eff] = AL_coef_loss[0][1]
        coef_MSE_loss[2, eff] = gAL_coef_loss[0][1]
        coef_MSE_loss[3, eff] = GWAL_coef_loss[0][1]
        coef_MSE_loss[4, eff] = GPAL_coef_loss[0][1]
        coef_MSE_loss[5, eff] = LWGAL_coef_loss[0][1]

        TP[0, eff] = AL_select[0][0]
        TP[1, eff] = gAL_select[0][0]
        TP[2, eff] = GWAL_select[0][0]
        TP[3, eff] = GPAL_select[0][0]
        TP[4, eff] = LWGAL_select[0][0]

        Model_Size[0, eff] = AL_select[0][1]
        Model_Size[1, eff] = gAL_select[0][1]
        Model_Size[2, eff] = GWAL_select[0][1]
        Model_Size[3, eff] = GPAL_select[0][1]
        Model_Size[4, eff] = LWGAL_select[0][1]
        ...
        Cindex[4, eff] = np.average(C_index_GPAL[eff, :])
        Cindex[5, eff] = np.average(C_index_LWGAL[eff, :])
        ...
        GPAL_coef, GPAL_intercept = np.array(GPAL_model.coef_.reshape((M, p))), np.array(GPAL_model.intercept_)
        LWGAL_coef, LWGAL_intercept = np.array(LWGAL_model.coef_.reshape((M, p))), np.array(LWGAL_model.intercept_)
        ...
        GPAL_fin = np.hstack((GPAL_coef, GPAL_intercept.reshape(-1, 1)))
        ...
        pd.DataFrame(GPAL_fin).to_excel(output_dir + "Coef_GPAL" + ".xlsx")
...
result['GPAL_coef_average_MAB_loss(var)'] = [
...
result['GPAL_coef_average_MSE_loss(var)'] = [
...
result['GPAL_tp(var)'] = [str(round(TP_average[3], 3)) + ' (' + str(round(TP_std[3], 3)) + ')']
result['LWGAL_tp(var)'] = [str(round(TP_average[4], 3)) + ' (' + str(round(TP_std[4], 3)) + ')']
...
result['GPAL_model_size(var)'] = [str(round(Model_Size_average[3], 3)) + ' (' + str(round(Model_Size_std[3], 3)) + ')']
result['LWGAL_model_size(var)'] = [str(round(Model_Size_average[4], 3)) + ' (' + str(round(Model_Size_std[4], 3)) + ')']
...
result['GPAL_average_Cindex(std)'] = [str(round(Cindex_average[4], 3)) + ' (' + str(round(Cindex_std[4], 3)) + ')']
result['LWGAL_average_Cindex(std)'] = [str(round(Cindex_average[5], 3)) + ' (' + str(round(Cindex_std[5], 3)) + ')']
